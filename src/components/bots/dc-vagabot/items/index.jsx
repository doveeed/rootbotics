...
export default function Items({items = [], onUpdateItems = () => {}}) {
    ...
    return (
        ...
        <Card title="Battle Track">
            <div style={{display: 'flex', flexDirection: 'column', marginBottom: '1rem'}}>
                <div style={{display: "grid", gap: '0.5rem',  gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', flexWrap: "wrap", width: '100%'}}>
                    {items.map(({key, isExausted, isDamaged}, index) => isBattleTrack(index) ? (
                        <div key={key} style={{position: "relative", userSelect: 'none', display: 'flex', flex: 1,}} >
                            ...
                            <div style={{width: '100%', height: '100%', display: 'flex', boxSizing: 'border-box', alignItems: "center", justifyContent: 'center', 
                                ...
                                userSelect: 'none',
                                overflow: 'hidden',
                                fontWeight: 'bold', borderRadius: '15%', border: `4px solid ${factionColor}`,position: "absolute", top: 0,}}><div style={{transform:'rotate(-45deg)', lineHeight: '1rem'}}>{isDamaged && (<>Damaged</>)}{isBattleTrack(index) && (<>Battle Track</>)}</div></div>
                        </div>
                    ): null)}
                </div>
            </div>
        ...
        <Card title="Satchel">
            <div style={{display: 'flex', flexDirection: 'column', marginBottom: '1rem'}}>
                <div style={{display: "grid", gap: '0.5rem',  gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', flexWrap: "wrap", width: '100%'}}>
                    {items.map(({key, isExausted, isDamaged}, index) => isBattleTrack(index) ? null :(
