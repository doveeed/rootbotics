...
const characters = {
    ...
    arbiter: {id: 'arbiter', name: 'Arbiter', special:  <>Before rolling in battle, the Defender may enlist the Arbiter if the Vagabot pawn is in the battle clearing. <i>(If other bots are in play, they will interact with the Arbiter, whether played by a human or bot, in the same way.)</i> The Arbiter scores <OneVP /> and adds the number of items on his Battle Track to the Defender's maximum rolled hits. Defending bots will enlist the Arbiter if they meet all three of the following conditions:
...
export default function DCVagabot({ state = {}, isRivetfolkPlaying, onDelete = () => {}, updateState = () => {}}) {
    ...
    const steps = {
        ...
        special: <Step title="Special." description={<>Exhaust 1 item to take the following action: {characters[character].special} Skip this action if it would have no effect.{totalActionItems - totalExhaustedOrDamagedItems > 0 && (<> <Button onClick={exhaustItem}>Exhaust item</Button></>)}{character === 'ranger' && totalDamagedItems > 0 ? (<> <Button onClick={() => repairItems(1)}>Repair 1 item</Button></>): ''}</>} />,
    }
